...
7. SteamDB extras (after Steam App ID): icon/cover from Steam `GetItems` (`community_icon` + `header` — same CDN URLs SteamDB embeds); optional scrape `steamdb.info/app/{id}/` for players (`@connect steamdb.info`); on miss fall back to Steam CDN `header.jpg` + `ISteamUserStats/GetNumberOfCurrentPlayers`.
