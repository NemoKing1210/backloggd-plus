...
7. Steam extras (after Steam App ID): icon/cover from Steam `GetItems` (`community_icon` + `header`); players from `GetNumberOfCurrentPlayers`. SteamDB has no public API — do not scrape steamdb.info (Cloudflare / private `/api/*`).
