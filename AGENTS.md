...
11. Cache in `GM_getValue` / `GM_setValue` (`blp_cache_v1`); settings in `blp_settings` (incl. per-link toggles). Cache successful Steam / GameStatus / SteamDB media / screenshots / HLTB / OpenCritic / ProtonDB hits only — do not persist misses or request errors. Player counts use a short TTL (~10 min).
