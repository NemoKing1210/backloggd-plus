...
10. Score enrichment (by title): OpenCritic search + game detail (`@connect api.opencritic.com`); HowLongToBeat bleed init + search (`@connect howlongtobeat.com`) with title match scoring.
11. Cache in `GM_getValue` / `GM_setValue` (`blp_cache_v1`); settings in `blp_settings` (incl. per-link toggles). Cache successful Steam / GameStatus / SteamDB media / HLTB / OpenCritic / ProtonDB hits only — do not persist misses or request errors. Player counts use a short TTL (~10 min).
12. UI strings in `TRANSLATIONS` / browser locale; settings via navbar **Plus** button (`btn btn-main`, same as **Log a Game**) and `GM_registerMenuCommand`.
